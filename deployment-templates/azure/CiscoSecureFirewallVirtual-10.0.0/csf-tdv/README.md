...
7. **customData**: The field to provide Day 0 configuration to the CSF-TDv. By default it has 4 key-value pairs to configure 'admin' user password, the CSF-TDv hostname, whether to use CSF-MCv or CSF-DM for management, and whether to use or not use the Diagnostic interface.<br>
...
e.g. {"AdminPassword": "Password@2023", "Hostname": "cisco", "ManageLocally": "Yes", "Diagnostic": "OFF"}<br>
...
e.g. {"AdminPassword": "Password@2023", "Hostname": "cisco", "ManageLocally": "No", "Diagnostic": "OFF", "FmcIp": "<fmcIp>", "FmcRegKey": "<fmcRegKey>", "FmcNatId": "<fmcNatId>" }<br>
...
12. **data1SubnetName**: The data1 interface will attach to this subnet. This maps to Nic1, the second subnet (Ethernet0/0 by default when Diagnostic is OFF). Note, this must match an existing subnet name if joining an existing network.<br>
e.g. data1

13. **data1SubnetIP**: The data1 interface IP address (Ethernet0/0 by default when Diagnostic is OFF).<br>
e.g. 10.8.1.55

14. **data2SubnetName**: The data2 interface will attach to this subnet. This maps to Nic2, the third subnet (Ethernet0/1 by default when Diagnostic is OFF). Note, this must match an existing subnet name if joining an existing network.<br>
e.g. data2

15. **data2SubnetIP**: The data2 interface IP address (Ethernet0/1 by default when Diagnostic is OFF).<br>
e.g. 10.8.2.55

16. **data3SubnetName**: The data3 interface will attach to this subnet. This maps to Nic3, the fourth subnet (Ethernet0/2 by default when Diagnostic is OFF). Note, this must match an existing subnet name if joining an existing network.<br>
e.g. data3

17. **data3SubnetIP**: The data3 interface IP address (Ethernet0/2 by default when Diagnostic is OFF).<br>
e.g. 10.8.3.55

18. **vmSize**: The VM size to use for the CSF-TDv VM. Standard_D8s_v3 is the default.<br>
...
21. **publicInboundPorts**: Whether public inbound ports are allowed or not.<br>
e.g. None or AllowSelectedPorts

22. **selectedInboundPorts**: Selected public inbound ports to be allowed in network security group.<br>
e.g. ["443", "22", "8305"]

23. **enableSecureBoot**: Enable Secure Boot for the VM (Trusted Launch feature).<br>
...
* [CSF-TDv deployment](https://www.cisco.com/c/en/us/td/docs/security/firepower/quick_start/consolidated_ftdv_gsg/threat-defense-virtual-10-0-gsg/m-ftdv-azure-gsg.html#id_82702)
